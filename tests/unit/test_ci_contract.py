...
def test_ci_workflow_runs_documented_quality_gates_and_docker_smoke() -> None:
    ...
    assert python_steps["Set up Python 3.14"]["with"]["python-version"] == "3.14"
    assert python_steps["Install uv"]["run"] == 'python -m pip install "uv==0.11.28"'
    assert python_steps["Install locked development environment"]["run"] == (
        "uv sync --locked --extra dev --extra distributed-spark"
    )
    assert python_steps["Ruff"]["run"] == "uv run ruff check ."
    assert python_steps["Mypy"]["run"] == "uv run mypy src/kg_processor tests"
    assert python_steps["Pytest"]["run"] == (
        "uv run pytest --cov=kg_processor --cov-report=term-missing"
    )
    ...
    assert docker_steps["Tear down smoke profile"]["run"] == (
        "docker compose -f .github/docker-compose.smoke.yaml down --volumes --remove-orphans"
    )
