...
from snowpark import RecordingManager, RecordingSession, Row
...
def test_listing_graphs_applies_the_visibility_predicate() -> None:
    ...
    listing = next(sql for sql in session.statement_texts if "FROM KG_JOB J" in sql)
...
def test_opening_a_graph_is_authorized_independently_of_the_listing() -> None:
    ...
    check = session.statement_texts[-1]
...
def test_sharing_rejects_a_target_that_is_neither_user_nor_role() -> None:
    ...
    with pytest.raises(ValueError, match="user or a role"):
        SnowflakeBackend(session).share_graph("graph-1", "EVERYONE", "public")

    assert session.statements == [], "the refusal must not reach SQL"
...
def test_claiming_a_graph_never_takes_one_that_is_already_owned() -> None:
    ...
    merge = session.statement_texts[-1]
...
def test_resolving_viewer_roles_escapes_the_username_it_must_interpolate() -> None:
    ...
    assert session.statement_texts[-1] == 'SHOW GRANTS TO USER "ALICE"" OR ""1""=""1"'
...
def test_a_run_id_belonging_to_another_graph_is_refused() -> None:
    ...
    session = RecordingSession(
        [
            ("FROM KG_GRAPH G WHERE G.GRAPH_ID", [Row({"1": 1})]),
            ("SELECT GRAPH_ID FROM KG_JOB", [Row({"GRAPH_ID": "alices-graph"})]),
        ]
    )
    backend = SnowflakeBackend(session)
    backend._viewer = Viewer(user_name="BOB")
...
def _backend_as(who: str) -> SnowflakeBackend:
    ...
    session = RecordingSession(
        [
            ...
            ("FROM KG_GRAPH G WHERE G.GRAPH_ID", []),
            ("SELECT 1 FROM KG_GRAPH WHERE GRAPH_ID", [Row({"1": 1})]),
            ("SELECT GRAPH_ID FROM KG_JOB", [Row({"GRAPH_ID": "alices-graph"})]),
        ]
    )
    backend = SnowflakeBackend(session)
...
@pytest.mark.parametrize(
    "act",
    [
        pytest.param(lambda backend: backend.cancel("alices-run"), id="cancel"),
        pytest.param(lambda backend: backend.forget("alices-run"), id="forget"),
        pytest.param(lambda backend: backend.rename_graph("alices-graph", "Renamed"), id="rename"),
    ],
)
def test_a_share_does_not_authorize_changing_the_owners_run(
    act: Callable[[SnowflakeBackend], object],
) -> None:
    """Keep a read share from acting on the owner's run.

    A shared run appears in the grantee's history with every control. If the
    control plane authorises on visibility, one click ends the owner's
    long-running, Cortex-billed run, removes it from their history, or renames
    the graph for everyone who can see it.
    """
...
def test_reading_someone_elses_quiet_run_never_writes_a_verdict_onto_it() -> None:
    ...
    assert not [s for s in backend.session.statement_texts if s.startswith("UPDATE KG_JOB")]
...
def test_a_stage_folder_belonging_to_another_viewer_cannot_be_listed() -> None:
    ...
    folder and ingest their documents into a graph of their own. The resolved
    location is authorised, not one form field: a stage identifier may carry a
    path of its own, and it is concatenated with the prefix before the LIST
    runs, so checking only the prefix let a viewer put another viewer's folder
    in the stage box and read it.
    """
...
def test_retrying_a_run_requeues_its_failed_documents_and_starts_a_worker() -> None:
    ...
    session = RecordingSession(
        [
            (
                "CONFIG:snowflake:compute_pool",
                [Row({"POOL": "POOL_A", "SPEC_STAGE": "@DB.SCH.SPECS", "DB": "DB", "SCH": "SCH"})],
            ),
            ("SELECT GRAPH_ID FROM KG_JOB", [Row({"GRAPH_ID": "graph-1"})]),
            ("COUNT(*) AS QUEUED", [Row({"QUEUED": 4})]),
            ("SELECT 1 FROM KG_GRAPH", [Row({"1": 1})]),
            ("FROM KG_JOB J", [job]),
            ("KG_JOB_FILE F", [job]),
        ]
    )
    ...
    statements = session.statement_texts
...
def test_a_running_worker_reports_what_it_has_spent_so_far() -> None:
    ...
    assert manager.written, "a stage transition is always written"
    assert manager.written[-1]["consumption"] == {
        "calls": 7,
        "total_tokens": 1234,
        "billed_usd": 0.5,
    }
...
def test_a_worker_that_cannot_price_its_work_still_reports_progress() -> None:
    ...
    assert manager.written[-1]["stage"] == "graph_extraction"
    assert "consumption" not in manager.written[-1]


def _running_run(quiet_for: int) -> Row:
    """One history row for a running job that last reported ``quiet_for`` seconds ago."""
...
def test_a_dead_worker_is_noticed_in_the_history_not_only_when_opened() -> None:
    ...
    backend = SnowflakeBackend(RecordingSession([("FROM KG_JOB J", [_running_run(7_200)])]))
    backend._viewer = Viewer(user_name="ALICE")
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(
...
def test_a_busy_run_is_not_interrogated_by_the_history() -> None:
    ...
    checked: list[str] = []
    backend = SnowflakeBackend(RecordingSession([("FROM KG_JOB J", [_running_run(3)])]))
    backend._viewer = Viewer(user_name="ALICE")
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(
            SnowflakeBackend,
...
def test_a_graph_deletion_is_refused_for_someone_elses_graph() -> None:
    ...
    # The ownership test finds no graph this viewer may administer, and the
    # graph is nonetheless known — so it belongs to somebody else.
    session = RecordingSession(
        [("FROM KG_GRAPH G", []), ("FROM KG_GRAPH WHERE GRAPH_ID", [Row({"1": 1})])]
    )
    backend = SnowflakeBackend(session)
    backend._viewer = Viewer(user_name="BOB")
...
def test_a_graph_deletion_removes_rows_from_every_graph_table() -> None:
    ...
    session = RecordingSession(
        [
            ("SELECT 1 FROM KG_GRAPH G", [Row({"1": 1})]),
            ("UNION ALL", [Row({"TABLE_NAME": "KG_NODE", "ROW_COUNT": 97})]),
        ]
    )
    backend = SnowflakeBackend(session)
    backend._viewer = Viewer(user_name="ALICE")
...
def _deleting_backend(*, upload_prefix: str) -> SnowflakeBackend:
    """Build ALICE's backend over her graph, whose one run read ``upload_prefix``."""
    ...
    session = RecordingSession(
        [("SELECT 1 FROM KG_GRAPH G", [Row({"1": 1})]), ("stage_prefix", [run])]
    )
    backend = SnowflakeBackend(session)
    backend._viewer = Viewer(user_name="ALICE")
    return backend
...
def test_deleting_a_graph_also_clears_its_staged_documents_and_service() -> None:
    ...
    backend = _deleting_backend(upload_prefix="app/ALICE/run-1")

    backend.delete_graph("graph-1")

    statements = backend.session.statement_texts
...
def test_a_curated_source_stage_is_never_emptied_by_a_deletion() -> None:
    ...
    backend = _deleting_backend(upload_prefix="corporate/reference-corpus")

    backend.delete_graph("graph-1")

    assert not any("corporate/reference-corpus" in s for s in backend.session.statement_texts)
