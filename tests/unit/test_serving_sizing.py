...
# fp8 cache, 21.81 GiB of weights, on a 119.2 GiB unified-memory device that
# holds about 12 GiB of activations, graphs, and allocator slack alongside them.
...
def test_the_shipped_default_leaves_the_sequence_limit_binding_first() -> None:
    verdict = compute_sizing(REFERENCE_GEOMETRY, _budget(0.60), 32768, max_num_seqs=32)

    assert verdict.max_concurrent == 37
...
def test_a_sequence_limit_at_or_above_capacity_is_reported_unsafe() -> None:
    ...
    at_capacity = compute_sizing(REFERENCE_GEOMETRY, budget, 32768, max_num_seqs=37)
