...
def test_build_snowflake_rows_maps_graph_batch() -> None:
    ...
    assert rows["KG_DOCUMENT"][0]["ID"] == "file_1"
    ...
    assert rows["KG_CHUNK"][0]["DOCUMENT_ID"] == "file_1"
...
def test_reindex_delete_statements_for_file_batch_are_file_scoped() -> None:
    ...
    assert (
        "DELETE FROM KG_EDGE WHERE GRAPH_ID = ? AND SOURCE_FILE_ID IN (?)",
        ["graph", "file_1"],
    ) in statements
    assert not any(sql == "DELETE FROM KG_NODE WHERE GRAPH_ID = ?" for sql, _ in statements)
    assert not any(sql == "DELETE FROM KG_COMMUNITY WHERE GRAPH_ID = ?" for sql, _ in statements)
