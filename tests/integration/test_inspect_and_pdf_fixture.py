...
def test_original_pdf_fixture_is_parseable() -> None:
    source = LocalFileSource(Path("data/samples/martial-arts-overview.pdf"))
    file = source.list_files()[0]

    parsed = BuiltinTextOcrProvider().parse(file, OcrOptions())

    assert parsed.pages
    assert sum(len(page.raw_text) for page in parsed.pages) > 1000


def test_original_text_native_office_and_html_fixtures_are_parseable() -> None:
    sample_paths = [
        Path("data/samples/martial-arts-timeline.html"),
        Path("data/samples/martial-arts-interview.docx"),
        Path("data/samples/martial-arts-schools.pptx"),
    ]
    ...
    for path in sample_paths:
        ...
        assert parsed.pages
        assert sum(len(page.raw_text.strip()) for page in parsed.pages) > 100
