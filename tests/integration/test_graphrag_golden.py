...
_GOLDEN_FIXTURE = Path("tests/fixtures/graphrag_spike/repeated_relation.json")


def test_graphrag_spike_repeated_relation_golden_fixture(tmp_path: Path) -> None:
...
def _assert_golden_batch(batch: GraphWriteBatch, expected: dict[str, Any]) -> None:
    ...
    assert {evidence.quote for evidence in batch.evidence} == {evidence_expectation["quote"]}
