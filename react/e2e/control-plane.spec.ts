...
test.describe("remaining report journeys", () => {
  ...
  test("sorts, searches, expands and pages the explore tables", async ({ page }) => {
    ...
    await page.getByRole("tab", { name: "Communities" }).click();
    const communities = page.getByRole("tabpanel", { name: "Communities" });
    await expect(communities.getByTestId("record-table-count")).toHaveText("10 communities");
    ...
    await page.getByRole("tab", { name: "Communities" }).click();
    await expect(communities.getByTestId("record-table-count")).toHaveText(/^\d+ communities \(of 10 total\)$/);
