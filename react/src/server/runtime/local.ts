...
export class LocalRuntime implements ControlPlane {
  ...
  forget(runId: string): Effect.Effect<void, ControlPlaneError> {
    return Effect.tryPromise({
      try: async () => {
        ...
        processRegistry.forget(runId);
        await removeRunDirectory(this.stateRoot, runId);
      },
      catch: (cause) => fromCause(cause, "Unable to forget run"),
    });
  }
  ...
  graphShares(_graphId: string): Effect.Effect<readonly GraphShare[], ControlPlaneError> {
    return Effect.succeed([]);
  }
