...
export interface InspectReport {
  ...
  quality: { ok: boolean; shareBlockedReason: string | null };
...
export function inspectFromDataset(
  ...
  return {
    ...
    quality: {
      ok: !quality.shareBlockedReason && !quality.zeroEntityFailure,
      shareBlockedReason: quality.shareBlockedReason,
    },
    gold: quality.gold,
  };
}
...
<body>
  ...
  <p>Quality ${report.quality.ok ? "accepted" : "blocked"}${report.quality.shareBlockedReason ? ` · ${escapeHtml(report.quality.shareBlockedReason)}` : ""}</p>
  <section>
    <h2>Top entities</h2>
