...
compatible.
A source entity marked is_document_subject is what the document itself is or
describes and may additionally be implicit through document provenance: an authorial passage of a
paper about its own method, experiment, result, or contribution, or a field or
table row below the header of a certificate, form, label, data sheet, or
specification that names the subject, may support it without repeating its name.
This exception is source-only. A quote given in parts joined by "..." is read as
those exact parts of one document. Treat a cited, historical, generic, or
unrelated statement as insufficient rather than assigning it to the subject.
