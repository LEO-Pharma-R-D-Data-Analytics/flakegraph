...
# Tables a graph written before they existed may lack; absence is not a
# schema failure, only presence with the wrong columns is.
_OPTIONAL_TABLES = frozenset({"discarded_windows", "rejected_records", "failed_documents"})
...
def _schema_checks(artifacts: dict[str, ArtifactTable]) -> dict[str, Any]:
    ...
    for table_name, required_columns in _REQUIRED_COLUMNS.items():
        ...
        optional_absence = table_name in _OPTIONAL_TABLES and not artifact.exists
        checks.append(
            {
                "table": table_name,
                "exists": artifact.exists,
                "ok": optional_absence or (artifact.exists and not missing_columns),
                "missing_columns": [] if optional_absence else missing_columns,
            }
        )
    return {
        "ok": all(check["ok"] for check in checks),
