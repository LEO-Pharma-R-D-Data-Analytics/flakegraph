...
def extract_graph_observations(
    ...
    return ExtractionObservations(
        entities=_dedupe_mentions([*entity_observations.entities, *relation_observations.entities]),
...
def _extract_window_relations(  # noqa: PLR0912
    ...
    if settings.verify_relations and relations_requiring_verification:
        ...
        for batch_index, batch in enumerate(
            (
                ...
                for batch in batched(
                    relations_requiring_verification, _RELATION_VERIFICATION_BATCH_SIZE
                )
            ),
            start=1,
        ):
        ...
        relations = verified
    return _WindowResult(entities=_dedupe_mentions(added), relations=relations, trace=trace)
