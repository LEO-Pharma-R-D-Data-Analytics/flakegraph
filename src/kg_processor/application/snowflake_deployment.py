...
from kg_processor.adapters.snowflake import split_stage_uri, validate_stage_location
...
def render_execute_job_service_sql(
    ...
    service_name = _qualified_identifier(settings.snowflake.service_name, "service name")
    ...
    return (
        ...
        f"{async_clause}\n"
        f"  FROM {spec_stage}\n"
        f"  SPEC = {_sql_string(safe_spec_file)};"
    )
            ...
            "template": {
                "metadata": {
                    "labels": {
                        ...
                        "app.kubernetes.io/instance": settings.job.job_id,
                    }
                },
                "spec": {
                    "restartPolicy": "Never",
...
def render_snowflake_setup_sql(
    ...
    statements.extend(_target_context_statements(database, schema))
    setup_sql = ";\n\n".join(statements) + ";"
...
def _spcs_environment(settings: Settings, config_path: str) -> dict[str, str]:
    env: dict[str, str] = {
        ...
        "KG_SNOWFLAKE_AUTHENTICATOR": settings.snowflake.authenticator or "oauth",
