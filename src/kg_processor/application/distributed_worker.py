...
from kg_processor.application.content_kinds import dataset_profiles, dataset_windows
...
class DistributedWorker:
    ...
    def _prepare_document(self, lease: TaskLease, run: _RunContext) -> TaskExecution:
        ...
        context_task = TaskDefinition(
            ...
            priority=self.settings.distributed.task_priority(TaskStage.EXTRACT_DOCUMENT_CONTEXT),
    ...
    def _extract_document_context(self, lease: TaskLease, run: _RunContext) -> TaskExecution:
        ...
        # A profiled dataset may need no windows, or entity windows only.
        extract_entities, extract_relations = dataset_windows(
            next(iter(dataset_profiles(contextualized.trace).values()), {}).get("kind")
        )
        windows = [
            window
        ...
        for window_batch in batched(windows, windows_per_task):
            ...
            follow_up_tasks.append(
                TaskDefinition(
                    ...
                    dependency_ids=[lease.task.id],
                    priority=self.settings.distributed.task_priority(
    ...
    def _compact_entity_inventory(self, lease: TaskLease) -> TaskExecution:
        ...
        compact_task = TaskDefinition(
            ...
            priority=self.settings.distributed.task_priority(TaskStage.COMPACT_DOCUMENT),
            max_attempts=self.settings.distributed.max_attempts,
        )
        return TaskExecution(
        ...
        compacted = ExtractedDocumentShard(
            ...
            observations=ExtractionObservations(
                entities=_unique_mentions(
                    [*inventory.entities, *(e for shard in relation_shards for e in shard.entities)]
                ),
