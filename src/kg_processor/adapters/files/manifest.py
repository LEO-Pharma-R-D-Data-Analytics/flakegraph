...
class ManifestFileSource:
    ...
    def list_files(self) -> list[InputFile]:
        ...
        return sorted(files, key=lambda file: file.source_uri)
...
def manifest_candidate_paths(manifest_path: Path, include_globs: list[str]) -> list[Path]:
    ...
    for row in _read_manifest_rows(manifest_path):
        ...
        if is_supported_file(path) and matches_include_globs(path_value, include_globs):
...
def _read_json_rows(path: Path) -> list[Mapping[str, Any]]:
    if path.suffix.lower() in {".jsonl", ".ndjson"}:
        return [
            _object_row(json.loads(line), path, line_number)
