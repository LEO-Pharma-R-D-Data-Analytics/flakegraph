...
def _write_snapshot(output_path: Path, batch: GraphWriteBatch) -> None:
    ...
    _write_parquet(
        output_path / "discarded_windows.parquet",
        _dump_models(batch.discarded_windows),
        columns=_DISCARDED_WINDOW_COLUMNS,
    )
    _write_parquet(
        output_path / "rejected_records.parquet",
        _dump_models(batch.rejected_records),
        columns=_REJECTED_RECORD_COLUMNS,
    )
    _write_parquet(
        output_path / "failed_documents.parquet",
        _dump_models(batch.failed_documents),
        columns=_FAILED_DOCUMENT_COLUMNS,
    )
